            ...
            {userLoaded &&
              (user ? (
                <div className="relative" ref={profileRef}>
                  ...
                  >
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] flex items-center justify-center text-sm font-bold text-black">
                      {user?.firstName?.[0] || "?"}
                      {user?.lastName?.[0] || ""}
                    </div>
                  </button>
                  {profileOpen && (
                    <div className="absolute right-0 top-full mt-2 w-56 py-1 rounded-xl bg-[rgba(13,26,45,0.95)] backdrop-blur-xl border border-white/10 shadow-xl z-50">
                ...
                <Link
                  href="/signup"
                  className="border-2 border-[var(--accent)] text-[var(--accent)] px-4 py-2 rounded-lg font-semibold text-sm hover:bg-[var(--accent)] hover:text-black transition-all duration-200"
                >
                  Lancer l&apos;App
                </Link>
              ))}
